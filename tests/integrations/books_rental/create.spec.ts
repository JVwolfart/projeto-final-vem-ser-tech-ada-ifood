...
describe("POST rental/books/", () => {
    ...
    it("should create rental and return the correct values", async() => {
        const {app} = sut();
        const response = await request(app).post("/v1/rental/books").send(newRental);
    ...
    it("should return 409 if the book is already rented", async() => {
        ...
        rentedBook.book_id = books[0].id;
        const response = await request(app).post("/v1/rental/books").send(rentedBook);
